<template>
  <div v-if="bookmarkUrl">
    ...
    <div v-if="metadata && !error" class="bg-white rounded-xl border border-gray-200 hover:border-violet-400 hover:shadow-md transition-all duration-200 group overflow-hidden">
      <a 
        :href="bookmarkUrl" 
        target="_blank" 
      ...
      >
        ...
        <div class="p-4">
          ...
          <div class="flex items-start gap-3 mb-3">
            ...
            <svg class="w-4 h-4 text-gray-400 group-hover:text-violet-600 group-hover:translate-x-0.5 transition-all shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
// Cache for bookmark metadata (in memory)
...
const fetchMetadata = async (url) => {
  ...
  if (metadataCache.has(url)) {
    metadata.value = metadataCache.get(url)
    return
  }
...
// Watch for URL changes and fetch metadata
watch(bookmarkUrl, (newUrl) => {
  if (newUrl) {
