        ...
        batch.map(async (row) => {
          ...
          try {
            ...
            if (activeTab.value === "bookmarks") {
              ...
              const url = row[csvHeaders.value.indexOf(mapping.url)];
              const note = mapping.note ? row[csvHeaders.value.indexOf(mapping.note)] : "";
              content = [
                title ? `#@title=${title}` : "",
                `#@bookmark=${url}`,
                note ? `#@note=${note}` : "",
              ...
              const title = mapping.title ? row[csvHeaders.value.indexOf(mapping.title)] : "";
              const username = mapping.username ? row[csvHeaders.value.indexOf(mapping.username)] : "";
              const email = mapping.email ? row[csvHeaders.value.indexOf(mapping.email)] : "";
              const password = row[csvHeaders.value.indexOf(mapping.password)];
              const totp = mapping.totpSecret ? row[csvHeaders.value.indexOf(mapping.totpSecret)] : "";
              const urls = mapping.urls ? row[csvHeaders.value.indexOf(mapping.urls)] : "";
              content = [
                title ? `#@title=${title}` : "",
    ...
    <div>
      ...
      <div class="mb-8 p-6 bg-white rounded-xl shadow-lg">
        <div class="mb-4 border-b border-gray-200">
          <button v-for="tab in ['bookmarks', 'passwords']" :key="tab" @click="activeTab = tab"
