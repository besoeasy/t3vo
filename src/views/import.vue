...
const parseCSV = () => {
  ...
  Papa.parse(file.value, {
    complete: (results) => {
      ...
      csvData.value = results.data.slice(1).filter(row => 
  ...
  <div class="m-auto p-6">
    ...
    <div class="mb-8 p-6 bg-white rounded-xl shadow-lg">
      ...
      <div class="mb-4 border-b border-gray-200">
        <button
          v-for="tab in ['bookmarks', 'passwords']"
          :key="tab"
          @click="activeTab = tab"
          :class="[
            'px-4 py-2 -mb-px',
        ...
        <div class="grid gap-4 md:grid-cols-2">
          <template v-if="activeTab === 'bookmarks'">
            <div v-for="field in ['title', 'url', 'note']" :key="field">
              ...
              <select
                :id="field"
                v-model="columnMapping.bookmarks[field]"
          ...
          <template v-else>
            <div v-for="field in ['title', 'username', 'email', 'password', 'totpSecret', 'urls']" :key="field">
              ...
              <select
                :id="field"
                v-model="columnMapping.passwords[field]"
                class="mt-1 block w-full pl-3 pr-10 py-2 text-base border-gray-300 focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm rounded-md"
                :disabled="isLoading"
              >
                <option value="">Select column</option>
